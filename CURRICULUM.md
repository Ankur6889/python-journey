...
- [~] The standard library — **OPENED S45 (Tue 8 Sep 2026, ~22:10): a folder of ordinary `.py` files, `/usr/lib/python3.12/` (202 entries), the fourth `sys.path` entry; `__file__` defined before use; `teaching/s45_stdlib/where_is.py` (`json.__file__` → `json/__init__.py`, so a package by the same route rule as `arm`) and `front_door.py` (`json/__init__.py` lines 106–108, `from .decoder import ...`, shown unedited as the REAL front door: one `import json` → four `sys.modules` keys, twelve names on `json`). Teach-back (why `arm` empty, `json` twelve) HELD for S46. Idea 3 (open and read one stdlib file) not yet taught.** **S46 (Thu 10 Sep 2026): RE-WALKED at his request ("start from where circular finished") — the route done BY HAND with `ls` down `sys.path` until `/usr/lib/python3.12/json/` appears; "a folder is not code" as the reason `__init__.py` runs; the front-door CHAIN read off disk (`json/decoder.py` line 5 `from json import scanner` is where `json.scanner` comes from); his question on key order answered with `teaching/s46_stdlib/key_order/` (early in, moved to the END on finish). IDEA 3 LANDED: `bisect.py` opened and `bisect_left` read line by line, every construct his except the bare `*` (defined, `star.py`); `enumerate()` defined (`enum.py`). Route-rule [RECALL] at 42h: gap named, not guessed. Still [~].** / [~] pip and third-party packages — **OPENED AND TAUGHT S46 (Thu 10 Sep 2026, `teaching/s46_pip/`): pip COPIES a folder into a folder already on `sys.path` (`site-packages` / `dist-packages`), no third tier; `where_third.py` — `json`, `numpy`, `rclpy` at entries 3, 6, 1, all `__init__.py`; SHADOWING shown live (`shadow/json.py` beats the stdlib at entry 0, crash lands on `.loads`, `AttributeError`); `sys.path.append` named as a smell, `pip install -e .` named as the fix (not shown). Teach-backs clean ("same folder then sys.path" corrected to: the folder IS `sys.path[0]`). Cold ask 12 Sep or later.** **1.10 IS NOW TAUGHT-COMPLETE; seven bullets wait on cold asks.**
- [~] What a package is — **OPENED S43 (Sun 6 Sep 2026): a folder holding `__init__.py`; `teaching/s43_packages/` (`use_arm.py`, `arm/__init__.py`, `arm/limits.py`); `import arm.limits` runs `__init__.py` THEN `limits.py`, binds ONE name `arm`, and `arm` IS the `__init__.py` module object; bound-name [PREDICT] missed (`arm.limits`); the closing teach-back is HELD for S44** **S44 (Mon 7 Sep 2026): CLOSED for the day — `import arm` alone runs `__init__.py` and STOPS (`only_arm.py` → `AttributeError`); `__init__.py` is normally EMPTY, or holds imports as the package's front door; `from arm import limits` loads the same, binds `limits`; `sys.modules` keys are fixed by disk position for every form. Bound-name miss REPEATED in the teach-back. Cold ask due 8 Sep.** **S45: bound name `arm` for `import arm.limits` said RIGHT unprompted (first time); `see_arm_ns.py` — `import arm` alone leaves `vars(arm)` empty, a submodule enters only when an import line names it. Cold ask re-dated 10 Sep.** / [~] Relative vs absolute imports — **OPENED S44 (Mon 7 Sep 2026): `arm/safety.py` (absolute, `from arm.limits import MAX_ANGLE`) vs `arm/safety_rel.py` (relative, `from .limits import MAX_ANGLE`); the dot = "my own folder", filled in from the module's dotted name; `arm/motion.py` + `run_motion.py` show the trap — run directly, the file is `__main__`, no dotted name, `ImportError: attempted relative import with no known parent package`. Teach-back: wrong absolute form (`import arm.limits` for `from . import limits`) and a surface answer on the trap. Cold ask due 8 Sep.** **S45 (7–8 Sep 2026): RE-WALKED at his request — probes `arm/p1.py`/`p2.py`/`p3.py` (same keys for every form, name follows the word after `import`); the ROUTE rule (`arm/direct.py`: same file loaded as `limits` from inside `arm/`, as `arm.limits` from outside — his words: "what decides is `sys.path[0]`, the base folder from where we start"); `arm/whoami.py` for `__main__` as a placeholder. Quiz: run-directly trap missed a SECOND time then reasoned; `run_safety_rel.py` predicted in full; `from . import limits` → `import arm.limits` a THIRD time, PARKED at his request with the answer given. Cold ask re-dated 10 Sep.**
- [~] Circular imports — **OPENED AND TAUGHT S45 (Tue 8 Sep 2026, ~21:30–22:05): `teaching/s45_circular/` (`a.py`, `b.py`, `main.py`, `self_check.py`, `run_self_check.py`, `fix/`). Frame: falls out of import-runs-the-file + the cache. `self_check.py` proves the key enters `sys.modules` BEFORE line 1 runs; so `b.py`'s `from a import X` HITS a half-built `a` → `ImportError: cannot import name 'X' from partially initialized module 'a'`. Not a loop — the early key prevents the loop. Two fixes: a third module (real), import inside the function (patch, `fix/b.py` run clean). Teach-backs clean after "infinite loop" corrected; his [PREDICT] on the unseen error label refused (pushback 87, upheld). Cold ask due 10 Sep.** / [~] sys.path — **OPENED S42 (5 Sep 2026, ~02:50): framed as a plain LIST of folders searched in order, first match wins, entry 0 = the script's folder; printed from his own machine (ROS Jazzy site-packages second); proof `import s22_counter` from the repo root → `ModuleNotFoundError`. The two-fixes [PREDICT] is unanswered — he stopped there.** **S43: taught to the ground after three restatements (physical-first: `sys` is a module, `path` a list inside it; slot 0 = the folder the SCRIPT lives in, `''` in a REPL; built fresh per process, not a history); [PREDICT] answered — `sys.path.append("/home/ankur/tools")` ✓ (with the `~`-not-expanded and append-before-import caveats), files-side-by-side declined and given, `import ~/tools/helper.py` → `SyntaxError`. Cold ask due 7 Sep.**
...
#### 1.12 OOP
**OPENED S48 (Sun 13 Sep 2026, 17:43 → 21:48). Prerequisite gate declared out
loud: 1.11 nine [~], 1.10 seven [~], opened on credit at his call. Eleven
files in `teaching/s48_oop/`, each pasted under its path with its command and
output and walked line by line; first student drill since S41. All
same-session; cold asks legal from 14 Sep evening.** **S49 (Mon 14 Sep 2026,
20:47 → ~21:00, stopped tired after ~15 min): three files in
`teaching/s49_oop/`; S49 material legal from 15 Sep evening.**
- [~] Why OOP exists — **S48: FRAME FIRST (a `class` adds a TYPE; ties the data shape and its functions under one name; buys `j.move(5)`, a real type, and readability of `nn.Module`/`Path`/exceptions; buys NOTHING a dict + functions cannot do — organisation plus a name). Teach-back: *"tight organisation and a separate datatype ... built on usual things we already use"* — right, the usual thing is a dict** / [~] Classes and instances — **S48: `make_one.py` (`class` binds a type; `Joint()` makes an object; `a is b` False; `__main__.Joint` read with 1.10 eyes); `print_one.py` (type + address, no contents); `attach.py` (attributes attached by dot; `AttributeError` NAMED by him before the traceback); `look_inside.py` (`vars(a)` — an object is a dict of attributes with a type stamped on it); `no_dict.py` from HIS question (`list().name = ...` → `AttributeError`; `vars([])` → `TypeError` naming `__dict__`; functions have one). Drill `drills/s48_joint.py` 10/10, same-session** / [~] `__init__` — **S48: `init.py` (the type call makes, then calls `__init__` with the new object as `self`; same address inside and outside); `init_short.py` (missing arg → `TypeError`, his [PREDICT] said `AttributeError`; discriminator: wrong CALL at the parentheses vs wrong DOT READ); `by_hand.py` from HIS question (`Joint(...)` returns the object; `__init__` returns `None`, discarded; `Joint.__init__(a, ...)` legal by hand; `Joint(a, ...)` → 4 vs 3 `TypeError`). Precision: "constructor" = the type call, `__init__` = initialiser**
- [~] Instance vs class attributes — **S48: `class_attr.py` delivered UNREAD. S49: RE-ISSUED in full and walked (assignment in the class block → `vars(Joint)`; object dicts carry no `unit`; `__init__` is in `vars(Joint)` too — methods are class attributes; the dot READ checks object dict then class dict). Held [PREDICT] `a.unit = "rad"`: lines 1–2 right (`rad`, `deg`); line 3 gave `vars(Joint)` for `vars(a)`, right after one pointer (*"which dict did the write go into"*). `teaching/s49_oop/shadow.py`: object-only WRITE, class copy untouched** / [~] Methods — instance, class, static — **S48, INSTANCE HALF ONLY: `method.py` (`a.move(5)` vs `Joint.move(a, 100)`, one function, the object decides whose dict); `bound.py` from his named gap (*"I don't know how python decides that"*): `Joint.move` is a `function`, `a.move` is a `bound method` holding `a`; `m = a.move; m(5)` works with no `a` in sight; lookup = object dict first, class dict second. Teach-back: *"the method stays with the class, and applies to whatever object it is called for."* Class/static methods NOT taught (`@` untaught). S49: CLASS HALF DELIVERED, NOT CONFIRMED READ — `teaching/s49_oop/from_dict.py` (frame: a second door in that runs before any object exists, so it gets the class not `self`; `cls` = the class left of the dot; `cls(...)` is the type call under another name; `@classmethod` given as SPELLING ONLY; honest frame: today only tidiness, the payoff lands at inheritance). [PREDICT] `print(Joint.from_dict)` posed, unanswered — he stopped tired. RE-ISSUE at S50. Static still not taught**
- [ ] Inheritance / [ ] Method overriding / [ ] super()
- [~] Dunder methods — **S49, `__repr__` ONLY: `no_repr.py` (address form, and a list of two addresses) → `with_repr.py` (returns the rebuild-call string `Joint('elbow', 10)`; it RETURNS, never prints; sits in `vars(Joint)` like `__init__`; the only special thing is WHO calls it). "Dunder" defined: double-underscore, a name Python looks for at a fixed moment — ties `__init__`, `__dict__`, `__file__`, `__name__`, `__main__`. [PREDICT] `print([a, b])` right (the list asks each element for its `__repr__`; same reason a printed list of strings shows quotes). `__str__` one line. Operator dunders untaught (1.13)** / [ ] Encapsulation / [ ] Polymorphism
